...
for course_code, course_group in course_groups:
    ...
    for index, row in course_group.iterrows():
        ...
        if chapter_name != current_chapter:
            # 创建章节点并记录顺序
            chapter_node = Node("Chapter", course_code=course_code, name=chapter_name, order=chapter_order)
            graph.merge(chapter_node, "Chapter", "name")  # 合并章节点，唯一标识为 `name`

            # 建立课程与章的关系
            rel_course_chapter = Relationship(course_node, "章节", chapter_node, order=chapter_order)
            graph.merge(rel_course_chapter)

            # 更新当前章信息
            current_chapter = chapter_name
            chapter_order += 1  # 章顺序递增
        ...
        # 创建知识点节点
        kp_node = Node("KnowledgePoint", name=knowledge_point)
        graph.merge(kp_node, "KnowledgePoint", "name")  # 合并知识点节点

        # 建立章与知识点的关系
        rel_chapter_kp = Relationship(chapter_node, "包含", kp_node, order=kp_order)
